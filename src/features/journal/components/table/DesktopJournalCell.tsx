...
import { getCellHoverColor } from "@/features/journal/utils/utils";
...
export const DesktopJournalCell = memo(function DesktopJournalCell({
  student,
  lesson,
  grade,
  type,
  ...
  return (
    <td
      className={`px-3 py-3 border-r dark:border-gray-700 text-center cursor-pointer transition-all min-w-[120px] relative ${
        ...
        quickMode && lesson.type === "lecture" ? "cursor-pointer" : ""
      } ${
        isToday ? `${lessonTodayColors.cell} ${lessonTodayColors.cellRing}` : ""
      }`}
      ...
      {grade && (
        <div className="flex flex-col items-center justify-center gap-1">
          ...
          <div className="flex gap-1">
            {hasComment && <MessageSquare className="h-3 w-3 text-blue-500" />}
          </div>
        </div>
      )}
      {isToday && (
        <div
          className={`absolute inset-0 ${lessonTodayColors.cellBorder} pointer-events-none rounded-md`}
        ></div>
      )}
    </td>
  );
