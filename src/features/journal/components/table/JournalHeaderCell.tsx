...
import {
  getLessonSpecificHeaderColor,
  getLessonSpecificTodayHighlightColors,
...
export const JournalHeaderCell = memo(function JournalHeaderCell({
  lesson,
  type,
  headerColor,
  currentDate,
}: JournalHeaderCellProps) {
  const isToday = isSameDate(lesson.date, currentDate);
  const lessonHeaderColor =
    type === "all" ? getLessonSpecificHeaderColor(lesson.type) : headerColor;
  const lessonTodayColors =
    type === "all"
      ? getLessonSpecificTodayHighlightColors(lesson.type)
      : getTodayHighlightColors(type);
  ...
  return (
    ...
    >
      <div className="text-sm font-semibold whitespace-nowrap flex items-center justify-center gap-1.5">
        {isToday && (
          <Calendar className={`h-4 w-4 ${lessonTodayColors.iconColor}`} />
        )}
        {lesson.date}
      </div>
      {type === "all" && (
      ...
      {isToday && (
        <div
          className={`absolute bottom-0 left-0 right-0 h-1 ${lessonTodayColors.indicator}`}
        ></div>
      )}
    </th>
  );
