...
import { getTodayHighlightColors } from "@/utils/dateUtils.ts";
...
export const calculateAttendanceRate = (
  ...
  const studentGrades = grades.filter(
    (g) => g.studentId === studentId && relevantLessonIds.has(g.lessonId),
  );
...
export const getHeaderColor = (type: string) => {
  switch (type) {
    ...
    case "all":
      return "bg-gray-100 dark:bg-gray-800/50"; // Колір для загального режиму
    default:
...
export const getCellHoverColor = (type: string) => {
  switch (type) {
    ...
    case "laboratory":
      return "hover:bg-purple-50 dark:hover:bg-purple-900/30";
    default:
...
export const getLessonSpecificHeaderColor = (
  lessonType: "lecture" | "practical" | "laboratory",
) => {
  return getHeaderColor(lessonType);
};

export const getLessonSpecificTodayHighlightColors = (
  lessonType: "lecture" | "practical" | "laboratory",
) => {
