...
export function ViewOptions({
    ...
    const [initialSettings, setInitialSettings] = useState({ viewMode, lessonsPerPage });

    const [isCustom, setIsCustom] = useState(!PRESET_VALUES.includes(String(lessonsPerPage)));
    const [customValue, setCustomValue] = useState(
        !PRESET_VALUES.includes(String(lessonsPerPage)) ? String(lessonsPerPage) : ''
    );
    ...
    const handlePopoverOpenChange = (open: boolean) => {
        if (open) {
            ...
            setCustomValue(!PRESET_VALUES.includes(String(lessonsPerPage)) ? String(lessonsPerPage) : '');
        } else if (
            ...
            <PopoverContent className="w-72" align="end">
                <div className="grid gap-6">
                    <div className="space-y-3">
                        <h4 className="font-medium leading-none text-base">Режим перегляду</h4>
                        <RadioGroup
                            value={viewMode}
                            onValueChange={(value) => setViewMode(value as 'pagination' | 'scroll')}
                        >
                            <div className="flex items-center space-x-3 p-2 rounded-md hover:bg-accent transition-colors">
                                ...
                                <Label htmlFor="r-pagination" className="font-normal flex items-center gap-2 cursor-pointer">
                            ...
                            <div className="flex items-center space-x-3 p-2 rounded-md hover:bg-accent transition-colors">
                                ...
                                <Label htmlFor="r-scroll" className="font-normal flex items-center gap-2 cursor-pointer">
                    ...
                    {viewMode === 'pagination' && (
                        <div className="space-y-3">
                            <h4 className="font-medium leading-none text-base">Кількість дат</h4>
                            <Select value={currentSelectValue} onValueChange={handleSelectChange}>
                                ...
                                <SelectContent>
                                    {PRESET_VALUES.map(val => (
                                        <SelectItem key={val} value={val}>{val} дат на сторінці</SelectItem>
                                    ))}
